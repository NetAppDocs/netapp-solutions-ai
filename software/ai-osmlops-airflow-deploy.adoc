...
Before you deploy Airflow, you must designate a default StorageClass within your Kubernetes cluster. The Airflow deployment process attempts to provision new persistent volumes using the default StorageClass. If no StorageClass is designated as the default StorageClass, then the deployment fails. To designate a default StorageClass within your cluster, follow the instructions outlined in the link:ai-osmlops-kubeflow-deploy.html[Kubeflow Deployment] section. If you have already designated a default StorageClass within your cluster, then you can skip this step.
...
image:aicp-010.png["Figure showing input/output dialog or representing written content"]


// NetApp Solutions restructuring (jul 2025) - renamed from ai/aicp_apache_airflow_deployment.adoc
