...
[.lead]
Before you can use Trident to dynamically provision storage resources within your Kubernetes cluster, you must create one or more Trident Backends. The examples that follow represent different types of Backends that you might want to create if you are deploying components of this solution on a link:../infra/ai-aipod-nv-intro.html[NetApp AIPod^]. For more information about Backends, and for example backends for other platforms/environments, see the link:https://docs.netapp.com/us-en/trident/index.html[Trident documentation^].
...
// NetApp Solutions restructuring (jul 2025) - renamed from ai/aicp_example_trident_backends_for_ontap_ai_deployments.adoc
