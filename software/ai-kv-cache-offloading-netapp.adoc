...
Use the same config file and hash seed so both instances agree on cache block identity and can read each other’s offloaded data from shared storage. Set the same `VLLM_API_KEY` on both instances so they accept requests from the router.
...
docker run -d --rm --name vllm-router \
  ...
  --static-backends "http://<instance1_host>:8001,http://<instance2_host>:8002" \
