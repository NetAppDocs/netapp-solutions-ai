...
- *Boost Inference Performance and GPU investment*
  This chart compares inference performance as conversation length and load increase. When KV cache is kept only in GPU memory, the system runs out of headroom quickly and throughput drops (indicated by the orange line).With multi-tier storage in place (GPU for active work, CPU for fast staging, and shared storage for durable, shareable cache), the platform accommodates more sessions and keeps performance from falling off as quickly. In other words, tiering helps you do more with the same GPUs, instead of hitting memory limits and recomputing the same work repeatedly.
