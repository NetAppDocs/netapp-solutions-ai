...
- *Enable a three-tier architecture* +
  A production-ready design combines:

  ** *GPU tier* — active cache for in-flight requests.
  ** *CPU tier* — fast local staging for recently used cache.
  ** *Shared storage tier* — durable, high-capacity cache shared across instances.
With this architecture, cache blocks can be promoted from shared storage into CPU memory on demand, while GPUs stay focused on active token generation.
