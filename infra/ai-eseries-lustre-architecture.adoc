...
After the client packages are installed, the optional `lustre_client` role in the link:https://github.com/NetApp/ansible-lustre/tree/release/1.0.0[ansible-lustre collection^] configures client network interfaces and LNet, enables multi-rail LNet when selected, validates connectivity, and manages a persistent systemd mount unit. The role does not build Lustre. It installs client packages only when `lustre_client_packages` is populated. If desired, clients can be configured manually. For step-by-step procedures, see link:ai-eseries-lustre-deploy.html[Deploy the solution] and the link:https://github.com/NetApp/ansible-lustre/blob/release/1.0.0/roles/lustre_client/README.md[lustre_client role documentation^].
