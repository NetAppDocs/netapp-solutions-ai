...
RAG applications involve retrieval of knowledge from companies' document repositories in various types such as PDF, text, CSV or Excel. This data is normally stored in solutions such as an S3 object storage or NFS on-premises as the source for data. NetApp has been a leader in data management, data mobility, data governance, and data security technologies across the ecosystem of edge, data center, and cloud. NetApp ONTAP data management provides enterprise-grade storage to support various types of AI workloads, including batch and real-time inferencing, and offers some of the following benefits: 
...
image:aipod-mini-002.png[300,300]
...
image:aipod-mini-003.png[300,300]
...
|2
|RAG inferencing nodes—with dual socket Intel Xeon 6900-series (96 core) or Intel Xeon 6700-series(64 core) processors and 250GB to 3TB RAM with DDR5(6400MHz) or MRDIMM (8800MHz). 2U server.
...
Figure 3 - AIPod Mini Deployment Architecture
image:aipod-mini-012.png[300,300]
...
|Kubernetes 1.31.9 (Installed by Enterprise RAG infrastructure playbook)
|Environment to run RAG framework 

|ONTAP  
|ONTAP 9.16.1P4 or above
|Storage OS on AFF A20. 
...
image:aipod-mini-004.png[600,600]
...
Figure 5 - SVM creation using ONTAP System Manager.
image:aipod-mini-005.png[600,600]
image:aipod-mini-006.png[600,600]
...
Figure 7 - Create an S3 bucket.
image:aipod-mini-008.png[600,600]
...
Figure 8 - S3 bucket permissions.
image:aipod-mini-009.png[600,600]
...
----
==== Install Kubernetes with Trident CSI driver
Install Kubernetes on your servers using Infrastructure playbook. Refer to the https://github.com/opea-project/Enterprise-RAG/blob/main/deployment/roles/infrastructure/netapp_trident_csi_setup/netapp_trident_integration.md[NetApp Trident CSI Integration for Enterprise RAG^] for details. 
...
==== Install and Configure MetalLB in your Kubernetes Cluster
Install MetalLB using helm in your Kubernetes Cluster. Refer to the https://metallb.io/installation/[MetalLB Installation^] for details.
Configure MetalLB with Layer2 configuration. Refer to https://metallb.io/configuration/#layer-2-configuration[MetalLB configuration^] for details.
...
In `inventory/<cluster-name>/config.yaml`, for the volume in the `pvc` list, change the `accessMode` to `ReadWriteMany`.
[source,cli]
----
gmc:
  enabled: true
...
To disable SSL certificate verification, set the following values in `inventory/<cluster-name>/config.yaml`
...
Access the OPEA for Intel® AI for Enterprise RAG UI. Refer to the https://github.com/opea-project/Enterprise-RAG/blob/release-2.0.0/deployment/README.md#interact-with-the-deployed-pipeline[Intel® AI for Enterprise RAG deployment documentation^] for details.

Figure 9 - OPEA for Intel® AI for Enterprise RAG UI.
image:aipod-mini-013.png[600,600]
...
Note:	At the time of this writing, the following file types are supported: PDF, HTML, TXT, DOC, DOCX, ADOC, PPT, PPTX, MD, XML, JSON, JSONL, YAML, XLS, XLSX, CSV, TIFF, JPG, JPEG, PNG, and SVG. 
...
Figure 10 - Data ingest UI.
image:aipod-mini-011.png[600,600]
...
_Note: The sizing guidance presented above is based on performance validation and test results gathered by using Intel Xeon 6 processors with 96 cores. For customers with similar I/O tokens and model size requirements we recommend using servers with Xeon 6 processors with 96 cores._
...
|Intel Xeon 6980P 2P 2G 504M 500W SGX512 
|4
...
For detailed deployment instructions, refer to the link:ai-aipod-mini-deployment-steps.adoc[NetApp AIPod Mini for ERAG - Deployment Steps]

== FAQs
Refer to the link:ai-aipod-mini-infra-readiness.adoc[NetApp AIPod Mini - Infra Readiness]
...
https://github.com/opea-project/Enterprise-RAG/tree/main/docs[Intel® AI ERAG Documentation]

https://github.com/opea-project/Enterprise-RAG/tree/main/deployment/playbooks[OPEA Enterprise RAG deployment playbook^]


// NetApp Solutions restructuring (jul 2025) - renamed from ai/ai-minipod.adoc
