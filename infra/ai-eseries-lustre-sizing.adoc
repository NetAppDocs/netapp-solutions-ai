...
Each EF80 array uses twenty-four NVMe drives. Supported capacities are 3.84 TB, 7.68 TB, and 15.3 TB with traditional volume groups (RAID 1 for MGS/MDT and RAID 6 for OST) or DDP, and 30.7 TB or 61.4 TB Capacity Flash (QLC) drives with DDP only. 1.92 TB drives are not currently recommended for this solution. For drive slot layout and pool selection, see link:ai-eseries-lustre-hw.html[Hardware components].
...
For drive layout and volume counts, see link:ai-eseries-lustre-hw.html[Hardware components]. For deployment steps, see link:ai-eseries-lustre-deploy.html[Deploy the solution]. For field-level inventory guidance, see link:ai-eseries-lustre-deploy-inventory.html[Customize the Lustre Ansible inventory].
