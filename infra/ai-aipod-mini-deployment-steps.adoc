...
=== 12. Update config.yaml file with volume access modes and SSL certificate verification
[source,cli]
----
vi inventory/<cluster-name>/config.yaml
gmc:
...
=== 13. Configure Schedule sync settings
[source,cli]
----
...
Note: Retrieve default UI credentials from cat ansible-logs/default_credentials.txt
