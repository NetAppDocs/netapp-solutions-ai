...
Modify the config.yaml file located at `inventory/<cluster-name>/config.yaml` to define the deployment FQDN, set the volume access modes, configure ingress exposure, and integrate ONTAP S3 as the Enterprise RAG data backend.

Edit `inventory/<cluster-name>/config.yaml` and apply the following configuration changes:
...
=== 13. Configure Schedule synchronization settings
