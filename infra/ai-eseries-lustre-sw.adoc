...
Use the link:https://wiki.whamcloud.com/display/PUB/Lustre+Support+Matrix[Lustre Support Matrix^] to find a compatible client operating system and kernel for your Lustre version. NetApp does not provide prebuilt Lustre client packages. Build client packages for the client operating system and kernel from the NetApp-provided Lustre source code in the link:https://github.com/NetApp/netapp-lustre[netapp-lustre repository^]. After the Lustre client packages are installed, the `lustre_client` role can configure network interfaces, LNet, and create a persistent systemd mount unit for the Lustre file system.
