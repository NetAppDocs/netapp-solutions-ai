...
| Data storage                           | AWS FSx ONTAP        | Refer to link:ai-mlops-fsxn-s3.html[Part 1 - Integrating Amazon FSx for NetApp ONTAP (FSx ONTAP) as a private S3 bucket into AWS SageMaker].
| Data science IDE                       | AWS SageMaker       | This tutorial is based on the Jupyter notebook presented in link:ai-mlops-fsxn-sagemaker.html[Part 2 - Leveraging Amazon FSx for NetApp ONTAP (FSx ONTAP) as a Data Source for Model Training in SageMaker].
...
+
image:mlops-fsxn-cictcd-002.png[Lifecycle configuration welcome page]
...
. This script executes the Jupyter Notebook, which handles the retraining and redeployment of the model for inference. After the execution is complete, the notebook will automatically shut down within 5 minutes. To learn more about the problem statement and the code implementation, please refer to link:ai-mlops-fsxn-sagemaker.html[Part 2 - Leveraging Amazon FSx for NetApp ONTAP (FSx ONTAP) as a Data Source for Model Training in SageMaker].
+
image:mlops-fsxn-cictcd-003.png[Create lifecycle configuration]
...
+
image:mlops-fsxn-cictcd-004.png[Update settings dropdown]

. Select the created *Lifecycle configuration* and click *Update notebook instance*.
+
image:mlops-fsxn-cictcd-005.png[Update lifecycle configuration of the notebook]
...
+
image:mlops-fsxn-cictcd-006.png[AWS lambda function welcome page]

. Please file all required entries on the page and remember to switch the Runtime to *Python 3.10*.
+
image:mlops-fsxn-cictcd-007.png[Create an AWS lambda function]
...
+
image:mlops-fsxn-cictcd-008.png[Select execution role]
...
+
image:mlops-fsxn-cictcd-009.png[Deployment]

. To specify how to trigger this AWS Lambda function, click on the Add Trigger button.
+
image:mlops-fsxn-cictcd-010.png[Add AWS function trigger]
...
+
image:mlops-fsxn-cictcd-011.png[Finalize trigger]
...
// NetApp Solutions restructuring (jul 2025) - renamed from ai/mlops_fsxn_cictcd.adoc
